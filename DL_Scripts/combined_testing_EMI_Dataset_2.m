...
Dictionary=[D_CFL D_LCD_CPU D_LC D_PRJ];
...
for i=1:4
    ...
    TEST = Data_Scaled(:,1:800); clear Data_Scaled;
%     TEST_Norm = (TEST - min(min(TEST)))/(max(max(TEST)) - min(min(TEST)));
    Z=myIST(Dictionary,TEST,1000,0.001);
    save(strcat('Z_',char(File_Names(i)),'_Selftest_T2.mat'),'Z');
